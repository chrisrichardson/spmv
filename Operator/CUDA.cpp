...
OperatorCUDA::OperatorCUDA(Eigen::SparseMatrix<double, Eigen::RowMajor>& A)
{
  ...
  cuda_CHECK(cudaMemcpy(value, A.valuePtr(), A.nonZeros() * sizeof(double),
                        cudaMemcpyHostToDevice));
  cuda_CHECK(cudaMemcpy(inner, A.innerIndexPtr(), A.nonZeros() * sizeof(int),
                        cudaMemcpyHostToDevice));
  cuda_CHECK(cudaMemcpy(outer, A.outerIndexPtr(), (A.rows() + 1) * sizeof(int),
                        cudaMemcpyHostToDevice));
...
{
  cuda_CHECK(cudaMemcpy(ydata, psp.data(), psp.size() * sizeof(double),
                        cudaMemcpyHostToDevice));
