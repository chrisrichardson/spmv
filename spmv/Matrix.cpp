...
Matrix<ScalarType, IndType>::Matrix(
    ...
    std::shared_ptr<spmv::L2GMap> col_maxp,
...
//-----------------------------------------------------------------------------
template <>
void Matrix<double, std::int32_t>::mkl_init()
{
...
//     std::vector<std::int64_t> row_ghosts, std::vector<std::int64_t>
//     col_ghosts)
// {
...
//   {
//     auto it
//         = std::upper_bound(row_ranges.begin(), row_ranges.end(),
//         row_ghosts[i]);
...
//   MPI_Neighbor_alltoallv(send_val.data(), send_size.data(),
//   send_offset.data(),
